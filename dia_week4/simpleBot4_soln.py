...
class Bot:
        ...
        for rr in registryActives:
            if isinstance(rr,Cat):
                if self.distanceTo(rr)<50.0:
                    playsound("385892__spacether__262312-steffcaffrey-cat-meow1.mp3")
                    collision = True
                    rr.jump()
        return collision
    ...
    def checkForDanger(self,dangerThreshold,registryActives):
        ...
        if max(sensors)>dangerThreshold:
            playsound("436589__julien-matthey__jm-transport-ext-horn-01a-car-short-mini-countryman.wav")
            for cc in registryActives:
                if isinstance(cc,Cat):
                    cc.bigJump()
...
class Cat:
    def __init__(self,namep,canvasp):
        ...
        imgFile = imgFile.resize((30,30), Image.BILINEAR)
...
def trainIt(canvas,registryActives,registryPassives,count,window,moves,trainingSet):
    moves += 1
    for rr in registryActives:
        if isinstance(rr,Bot):
        ...
        if moves>numberOfMoves:
            ...
            window.destroy()
            return
    canvas.after(1,trainIt,canvas,registryActives,registryPassives,count, window, moves, trainingSet)

def moveIt(canvas,registryActives,registryPassives,count,window,moves,dangerThreshold,collisions):
    moves += 1
    for rr in registryActives:
        if isinstance(rr,Bot):
            seen = rr.look(registryActives)
        ...
        if moves>numberOfMoves:
            print("total dirt collected in",numberOfMoves,"moves is",count.dirtCollected)
            window.destroy()
            print("Collsions with warnings ",collisions)
            return
    canvas.after(1,moveIt,canvas,registryActives,registryPassives,count, window, moves, dangerThreshold, collisions)
...
def train():
    ...
    moves = 0
    trainingSet = []
    trainIt(canvas,registryActives,registryPassives, count, window, moves, trainingSet)
    window.mainloop()
    print(trainingSet)
    print("*******************")
    warningValues = []
    for idx,tt in enumerate(trainingSet):
        if tt[1]==True and idx>5:
            warningValues.append(trainingSet[idx-5][0])
    dangerThreshold = 0.0
    count = 0
    for ww in warningValues:
        m = max(ww)
        print(m)
        if not m==0.0:
            dangerThreshold += m
            count += 1
    dangerThreshold /= count
    return dangerThreshold

def main(dangerThreshold):
    window = tk.Tk()
    canvas = initialise(window)
    registryActives, registryPassives, count = register(canvas)
    moves = 0
    collisions = 0
    moveIt(canvas,registryActives,registryPassives, count, window, moves, dangerThreshold, collisions)
    window.mainloop()

#dangerThreshold = train()
#print(dangerThreshold)
#main(dangerThreshold)
main(0.7897)
